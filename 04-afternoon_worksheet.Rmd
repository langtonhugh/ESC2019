...
```{r, warning = F}
centroits_sf <- st_centroid(manc_sf) # obtain centroids
```

And plot them as we have previously.

```{r}
ggplot(data = centroits_sf) +
  geom_sf()
```
...
```{r}
centroids_buff_sf <- st_buffer(centroits_sf, dist = 500)
```
